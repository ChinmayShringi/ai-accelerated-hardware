...
class Regs(AttrDict2):
    RUN: Reg = Reg('RUN', 1, 1)
    RESET0: Reg = Reg('RESET0', 1, 1)
    RESET1: Reg = Reg('RESET1', 1, 1)
    RESET2: Reg = Reg('RESET2', 1, 1)
    TIMEOUT_BIT: Reg = Reg('TIMEOUT_BIT', 5, 0)
    LOAD: Reg = Reg('LOAD', 1, 1)
    START_TIMER: Reg = Reg('START_TIMER', 1, 1)
    RESTART_TIMER: Reg = Reg('RESTART_TIMER', 1, 1)
    STOP_TIMER: Reg = Reg('STOP_TIMER', 1, 1)
    RECORD_TIMER: Reg = Reg('RECORD_TIMER', 1, 1)
    GET_RESULTS: Reg = Reg('GET_RESULTS', 1, 1)
    STALL: Reg = Reg('STALL', 1, 0)
...
class Layer(AttrDict2):
    ...
    layerio_wr_instrucs: MemInstrucs = init(lambda: MemInstrucs('layerio_wr_instrucs'))
    layerio_rd_instrucs: MemInstrucs = init(lambda: MemInstrucs('layerio_rd_instrucs'))
    weight_rd_instruc: MemInstruc = init(lambda: MemInstruc('weight_rd_instruc'))
    post_gemm_params_rd_instruc: MemInstruc = init(lambda: MemInstruc('post_gemm_params_rd_instruc'))
    post_gemm_params: PostGemmParams = init(lambda: PostGemmParams())
    weight: DataInstruc = init(lambda: DataInstruc('weight'))
    #
    Name = LowerStrEnum('LayerName', ' '.join([
